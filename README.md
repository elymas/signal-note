...
1. Aside에서 릴스 탭을 더 이상 고유 ID가 늘지 않을 때까지 스크롤하고 중복 제거한 인벤토리를 저장합니다.
2. 자동 자막이 없거나 5분 이상 영상의 자막 밀도가 분당 40단어 미만이면 로컬 Whisper 전사를 우선합니다. 무음 영상만 게시문과 대표 프레임을 근거로 제한적으로 분석합니다.
3. `npm run reels:generate-transcripts -- --source=<slug> --artifact=<path>`로 전사·화면 근거를 한국어 카드로 변환합니다. 이 명령은 로컬 `mlx-lm` 환경과 `mlx-community/Qwen2.5-7B-Instruct-4bit` 모델을 사용합니다.
4. 제목·핵심·실행 규칙·검증 메모는 영상 내용만 간결한 한국어로 작성합니다. 원문 전사 복사, 수집·스크롤·전사 과정, 고유 ID 등록 같은 작업 메모는 공개 카드에 넣지 않습니다.
5. `npm run reels:progress`, `npm run reels:verify-quality`, `npm run build`와 Aside 실페이지 점검을 모두 통과한 뒤 배포합니다.
